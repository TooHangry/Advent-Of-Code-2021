class BingoBoard:
    def __init__(self, board_string, name):
        self.board = []
        self.name = str(name)
        self.has_had_bingo = False
    ...
    def set_has_had_bingo(self):
        self.has_had_bingo = True

    def get_has_had_bingo(self):
    ...
    def has_bingo(self):
        ...
        for row in self.board:
            ...
            j += 1
            for entry in row:
                valid += ' 0 ' if not entry['marked'] else ' ' + \
                    str(entry['val']) + ' '
...
def get_boards():
    ...
    for line in file:
        ...
        if len(line) < 2:
            boards.append(BingoBoard(current, b))
            current = ''
            b += 1
    return boards
